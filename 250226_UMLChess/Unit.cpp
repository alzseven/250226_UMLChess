...
Queen::Queen(std::string name, char symbol, Team team, int x, int y)
	: Unit(name, symbol, team, x, y)
{
}

bool Queen::CanMove(int x, int y, class Board* chessBoard)
{
	int distX = x - GetX();
	int distY = y - GetY();
	
	// ???? ????? ??? ??? ?????? ???? ????
	int dx = distX == 0 ? 0 : distX < -1 ? -1 : 1;  // x?? ?????? ?????? dx?? 0
	int dy = distY == 0 ? 0 : distY < -1 ? -1 : 1;  // y?? ?????? ?????? dy?? 0
